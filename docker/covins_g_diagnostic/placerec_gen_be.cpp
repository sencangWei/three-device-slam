...
auto PlaceRecognitionG::ComputeSE3() -> bool {
    ...
    std::cout << "[COVINS_DIAG] compute query=" << kf_query_->id_.first
              << ":" << kf_query_->id_.second
              << " candidates=" << nInitialCandidates << std::endl;
    ...
    for (size_t i = 0; i < nInitialCandidates; i++) {
        ...
        std::cout << "[COVINS_DIAG] descriptor query=" << kf_query_->id_.first
                  << ":" << kf_query_->id_.second << " candidate="
                  << pKF->id_.first << ":" << pKF->id_.second
    ...
    for (size_t i = 0; i < nInitialCandidates; ++i) {
        ...
        if (!foundRelTransform) {
          std::cout << "[COVINS_DIAG] relpose_reject query="
                    << kf_query_->id_.first << ":" << kf_query_->id_.second
                    << " candidate=" << pKFi->id_.first << ":"
                    << pKFi->id_.second << std::endl;
          vbDiscarded[i] = true;
          continue;
        }
        ...
        std::cout << "[COVINS_DIAG] relpose_ok query=" << kf_query_->id_.first
                  << ":" << kf_query_->id_.second << " candidate="
                  << pKFi->id_.first << ":" << pKFi->id_.second
                  << " yaw_deg=" << mrelative_yaw << " trans_m="
                  << T_smatch_squery.block<3, 1>(0, 3).norm()
                  << " cov_trace=" << cov_loop.trace() << std::endl;
...
auto PlaceRecognitionG::DetectLoop()->bool {
    ...
    std::cout << "[COVINS_DIAG] query=" << kf_query_->id_.first << ":"
              << kf_query_->id_.second << std::endl;

    if(kf_query_->id_.first < covins_params::placerec::start_after_kf) {
        std::cout << "[COVINS_DIAG] skip_start query=" << kf_query_->id_.first
                  << ":" << kf_query_->id_.second << std::endl;
        kf_query_->SetErase();
        return false;
    }
    ...
    std::cout << "[COVINS_DIAG] database query=" << kf_query_->id_.first
              << ":" << kf_query_->id_.second << " min_score=" << minScore
              << " candidates=" << vpCandidateKFs.size() << std::endl;
    ...
    std::cout << "[COVINS_DIAG] consistency query=" << kf_query_->id_.first
              << ":" << kf_query_->id_.second << " groups="
              << mvConsistentGroups.size() << " enough="
              << mvpEnoughConsistentCandidates.size() << std::endl;
