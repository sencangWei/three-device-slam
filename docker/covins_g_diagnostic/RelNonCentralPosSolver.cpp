...
bool RelNonCentralPosSolver::computeNonCentralRelPose(
  ...
  for (size_t i = 0; i < n_qkfs; ++i) {
    ...
    for (size_t j = 0; j < n_ckfs; ++j) {
      ...
      if (matches_QC.size() < mImgMatches) {
          std::cout << "[COVINS_DIAG] view_match_reject q="
                    << curr_QKF->id_.first << ":" << curr_QKF->id_.second
                    << " c=" << curr_CKF->id_.first << ":"
                    << curr_CKF->id_.second << " matches="
                    << matches_QC.size() << " required=" << mImgMatches
                    << std::endl;
          return false;
      }
      ...
      if (this->computePose(curr_QKF, curr_CKF, matches_QC, threshold, T12_5pt,
        ...
        std::cout << "[COVINS_DIAG] five_point_reject q="
                  << curr_QKF->id_.first << ":" << curr_QKF->id_.second
                  << " c=" << curr_CKF->id_.first << ":"
                  << curr_CKF->id_.second << " matches="
                  << matches_QC.size() << " inliers=" << inlierInd_QC.size()
                  << " required=" << mMinInliers << std::endl;
        return false;
      }
    }
  ...
  opengv::relative_pose::FrameNoncentralRelativeAdapter adapter(
      view_A, view_B, match_vect, TF_vect, inliers_vect, T_init);
        ...
        std::cout << "[COVINS_DIAG] seventeen_point_reject inliers="
                  << sacProb.inliers_.size() << " required="
                  << mMinInliers_17PT << std::endl;
        return false;
        }
        ...
        if (cov_loop.trace() < mMax_cov) {
          std::cout << "[COVINS_DIAG] covariance_ok trace="
                    << cov_loop.trace() << " limit=" << mMax_cov << std::endl;
          return true;
        }
        std::cout << "[COVINS_DIAG] covariance_reject trace="
                  << cov_loop.trace() << " limit=" << mMax_cov << std::endl;
